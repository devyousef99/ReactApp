...
import { resetTblemailtrackingToInit, setTblemailtrackingMessage } from "redux/actions";
import { Button, Card, Col, Form, InputGroup } from "react-bootstrap";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteTblemailtracking } from "services/tblemailtrackingService";
type Props = {
    hideShowForm: (action) => void;
    handleRowEdit: (row) => void;
    getData: (page, pageSize, searchKey) => void;
};
export const TblemailtrackingTable: React.FC<Props> = ({ hideShowForm, handleRowEdit, getData }) => {
    ...
    useEffect(() => {
        if (rData && rData.list && rData.list.length === 0) {
            dispatch(resetTblemailtrackingToInit());
    ...
    const handleServerDelete = async () => {
        if (rowData) {
            const response = await deleteTblemailtracking(rowData.id);
            if (response) {
                dispatch(resetTblemailtrackingToInit());
                dispatch(setTblemailtrackingMessage("Deleted Successfully"));
                getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                setShowDelete(false);
            } else {
    ...
    return (
        <Card className="shadow mb-4">
            <Card.Header className="py-3">
                <h6 className="m-0 font-weight-bold text-primary">Tblemailtracking List ({rData.totalCount})
